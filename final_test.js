// Comprehensive test script for date responses
...
// The final function from index.js
function handleDateSpecificWorkInquiry(message, dateObj, simulateIsToday = false) {
  ...
  let isToday;
  if (simulateIsToday) {
    isToday = simulateIsToday;
  } else {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const dateObjCopy = new Date(dateObj);
    dateObjCopy.setHours(0, 0, 0, 0);
    isToday = today.getTime() === dateObjCopy.getTime();
  }
  
  console.log(`Is today: ${isToday}${simulateIsToday ? ' (simulated)' : ''}`);
...
console.log("=== SCENARIO 1: TODAY - NOT WORKING (March 18, 2025) ===");
const today = dateUtils.extractDateFromString("03/18/2025");
handleDateSpecificWorkInquiry(mockMessage, today, true); // Simulate as today

console.log("\n=== SCENARIO 2: TODAY - WORKING (March 19, 2025, simulated as today) ===");
const todayWorking = dateUtils.extractDateFromString("03/19/2025");
handleDateSpecificWorkInquiry(mockMessage, todayWorking, true); // Simulate as today

console.log("\n=== SCENARIO 3: FUTURE - NOT WORKING (March 18, 2025, treated as future) ===");
const futureNotWorking = dateUtils.extractDateFromString("03/18/2025");
handleDateSpecificWorkInquiry(mockMessage, futureNotWorking, false); // Treat as future

console.log("\n=== SCENARIO 4: FUTURE - WORKING (March 19, 2025) ===");
const futureWorking = dateUtils.extractDateFromString("03/19/2025");
handleDateSpecificWorkInquiry(mockMessage, futureWorking, false); // Real future date